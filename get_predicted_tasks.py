...
for i in range(len(problems_info)):
...
for i in range(len(final_abilities_database)):
    ...
    for task in all_tasks:
        ...
        range_of_tasks.append(
            [abs(similarity_for_task - final_abilities_database.iloc[i][f'predicted_similarity']), task])
    range_of_tasks.sort()
...
if list_for_demonstration:
    ...
    max_abilities = {task[1]: None for task in list_for_demonstration}
    ...
    for task in task_abilities:
        ...
        for j, theme in enumerate(themes_of_tasks):
            if problems_info.loc[id_of_tasks[task], theme] == 1:
                ...
                if abilities[j] > max_value:
                    max_value = abilities[j]
                    max_abilities[task] = j
    ...
    for idx, task in enumerate(task_abilities):
        if min_abilities[task] is not None:
            plt.scatter(min_abilities[task], abilities[min_abilities[task]], color=colors[idx],
                        s=50 * (len(task_abilities) - idx), marker='v', label=f"Task {task} (min)")
        if max_abilities[task] is not None:
            plt.scatter(max_abilities[task], abilities[max_abilities[task]], color=colors[idx],
                        s=50 * (len(task_abilities) - idx), marker='^', label=f"Task {task} (max)")

    plt.xlabel("Ability Index")
    plt.ylabel("Normalized Ability Value")
    plt.title(f"Abilities of user with Task Associations (Min and Max Highlighted)")
    plt.legend()
    plt.grid(True)
    plt.show()
